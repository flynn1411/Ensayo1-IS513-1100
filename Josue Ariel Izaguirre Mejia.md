> ### IS-513 Lenguajes de Programación (Sección 1100) <br> Josué Ariel Izaguirre Mejía <br> jaizaguirrem@unah.hn <br> 20171034157
...
Los lenguajes de programación han facilitado la comunicación con las computadoras. Su historia, carácteristicas, procesos de implementación y más se verán en este documento.
...
Al final, toda computadora entiende unicamente código máquina, por lo que ejecutar instrucciones de un lenguaje de alto nivel a uno de bajo nivel puede realizarse de ciertas máneras. La elección de utilizar código máquina como la comunicación directa hacía la computadora y sus componentes de hardware tiene que ver con la flexibilidad de lo que se puede hacer con la misma. El código máquina permite tener un control directo hacia el hardware y todas las capacidades de sus unidades de procesamiento. Existen tres tipos de implementación que son los más utilizados:

1. *Implementación de Compilación:*

    Este tipo de implementación es conocido por tener una ejecución rápida, al costo del tiempo que toma la compilación. El proceso de compilación generalmente contiene las siguientes fases:

    1. Se obtiene el código fuente, o las instrucciones escritas en el lenguaje de alto nivel.

    2. Un analizador léxico agrupa los carácteres del programa en unidades léxicas que pertenecen a los lexemas del lenguaje. Ignora los comentarios.

    3. Un analizador sintáctico crea, en base a las unidades léxicas, estructuras (en base a  gramáticas) tales (tablas generalmente) que se podría construir arboles de parseo cuando se necesite.

    4. Se genera una versión del código fuente que puede ser ensamblador o un tipo de lenguaje parecido al ensamblador, llamada código intermedio, en base a la estructura generada por el analizador sintáctico (en estas fases intermedias, la estructura es usada múltiples veces).

    5. Un analizador semántico busca cualquier posible error en la semántica de las instrucciones, en el caso que el analizador sintáctico no los haya detectado.

    6. Se optimiza el código al hacerle cambios que resultan en mayor velocidad en ejecución o tamañano en memoria.

    7. Un generador de código utiliza la estructura generada por el analizador sintáctico y el código optimizado para poder generar un código máquina para poder ser ejecutado por la máquina en la que se compila. Esto incluye obtener programas externos de librerías necesarias para lograr la ejecución del programa.

El resultado final es un programa. Este tipo de lenguajes contienen dos archivos, el código fuente y el ejecutable. Los ejecutables son programas rápidos y mantienen privado el código fuente, pero tienen ciertos problemas. Solo pueden ser ejecutados por la máquina que los compiló por lo que la portabilidad no es posible. Otro problema que surge es la lentitud de traducir todo el código fuente al ejecutable, lo cual hace tediosa la revisión de erroes por parte del diseñador.

2. *Implementación Pura:*

La implementación pura es el opuesto a la implementación de compilación. En lugar de realizar un proceso de compilación y obtener un ejecutable, el código fuente se lee y ejecuta linea por linea en una máquina virtual capaz de ejecutar dicho lenguaje en esos altos niveles. Este tipo de implementación logra resolver los problemas de la compilación, al costo de ser lento en ejecución lo cual lo convierte en el opuesto total de la implementación de compilación. Otra cosa que tomar en cuenta es no poder mantener privado el código fuente, ya que el archivo que se ejecutará será el código fuente.
